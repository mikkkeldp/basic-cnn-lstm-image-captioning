...
We solve this by maximizing the probability of the correct description given the image. In the training phase we want to find the best model's parameters &theta;* which satisfies 

![formula](https://render.githubusercontent.com/render/math?math=\theta*=\underset{x}{argmax}\sum_{(I,S)}logp(S|I,\theta))

where &theta; are the parameters of our model, *I* the image, and *S* is its correct caption.

The probability of the correct caption *S* given an image *I* can be modeled as the joint probability over its words *S<sub>0</sub>, S<sub>1</sub>, ... ,S<sub>N</sub>*:

![formula](https://render.githubusercontent.com/render/math?math=logp(S|I)=\sum^N_{t=0}logp_t(S_t|I,S_0,...,S_{t-1}))

where *N* is the length of the caption S, *S<sub>N</sub>* is a special end-of-sequence word (EOS), and the relationship between S and S<sub>i</sub> is defined by:

![formula](https://render.githubusercontent.com/render/math?math=S=Concatenate(S_0,S_1,...,S_N))

We then use a LSTM to model the joint probability distribution. Before that, we first need to encode images and captions into a fixed-length vector. -->
...
<p align="center">
<img src="alternative_model.png" width="550" height="400"/>
</p>
